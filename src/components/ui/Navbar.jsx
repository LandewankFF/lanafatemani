...
export const Navbar = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 transition-colors duration-300 ${isScrolled ? "bg-white" : "bg-transparent"}`}>
      <nav className="flex items-center px-20 xl:justify-between md:px-10 md:justify-between s:px-5 s:justify-between py-4">
        ...
        <div
          className={`font-medium absolute lg:static left-0 top-[80px] md:w-[550px] s:w-full bg-blue-300 lg:bg-transparent transition-transform duration-300 ease-in-out ${
          ...
          <ul className="lg:flex lg:flex-row lg:static lg:justify-center lg:items-center text-lg lg:gap-5 md:flex md:flex-col md:px-10 py-5 s:px-5">
            {navLink.map((menu) => (
              <li key={menu.name} className="mb-4 lg:mb-0 s:text-center">
                <Link
                  to={menu.path}
                  className={`hover:text-secondary duration-300 ease-in-out ${isScrolled ? "text-black" : "text-basic_white"} ${
